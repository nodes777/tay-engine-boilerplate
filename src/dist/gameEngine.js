...
gEngine.Core = (function() {
	...
	var clearCanvas = function(color) {
		...
		mGL.clear(
			mGL.COLOR_BUFFER_BIT | mGL.STENCIL_BUFFER_BIT | mGL.DEPTH_BUFFER_BIT
		);
	};
...
gEngine.VertexBuffer = (function() {
	...
	var verticesOfLine = [0.5, 0.5, 0.0, -0.5, -0.5, 0.0];
	...
	var initialize = function() {
		var gl = gEngine.Core.getGL(); // Step A: Create a buffer on the gGL context for our vertex positions

		//stores the reference for the GPU
		mSquareVertexBuffer = gl.createBuffer(); // Step B: Activate vertexBuffer

		//gl is canvas context
		gl.bindBuffer(gl.ARRAY_BUFFER, mSquareVertexBuffer); // Step C: Loads verticesOfSquare into the vertexBuffer
		...
		mTextureCoordBuffer = gl.createBuffer(); // Activate vertexBuffer

		gl.bindBuffer(gl.ARRAY_BUFFER, mTextureCoordBuffer); // Loads verticesOfSquare into the vertexBuffer
		...
		gl.bufferData(
			gl.ARRAY_BUFFER,
			new Float32Array(verticesOfLine),
			gl.STATIC_DRAW
		);
	};
...
gEngine.Input = (function() {
	...
	var _onMouseMove = function(event) {
		...
		var x = Math.round(
			(event.clientX - bBox.left) * (mCanvas.width / bBox.width)
		);
		var y = Math.round(
			(event.clientY - bBox.top) * (mCanvas.width / bBox.width)
		);
...
gEngine.GameLoop = (function() {
	...
	var _runLoop = function() {
		if (mIsLoopRunning) {
			...
			mLagTime += mElapsedTime; // Step C: update the game the appropriate number of times. // Update only every Milliseconds per frame. // If lag larger then update frames, update until caught up.
	...
	var _startLoop = function() {
		...
		mLagTime = 0.0; // Step B: remember that loop is now running
		mIsLoopRunning = true; // Step C: request _runLoop to start when loading is done
		requestAnimationFrame(function() {
			_runLoop.call(mMyGame);
		});
	};
...
gEngine.Physics = (function() {
	...
	var resolveCollision = function(s1, s2, collisionInfo) {
		...
		mHasOneCollision = true; // Step B: correct positions

		_positionalCorrection(s1, s2, collisionInfo); // Step C: apply friction
...
gEngine.Particle = (function() {
	...
	var resolveRectPos = function(rectShape, particle) {
		...
		if (collided) {
			...
			mVec[0] = mFrom1to2[0];
			mVec[1] = mFrom1to2[1]; // Find closest axis

			if (
				Math.abs(mFrom1to2[0] - alongX) <
				Math.abs(mFrom1to2[1] - alongY)
			) {
				...
				// y axis is shorter
				mNormal[0] = 1;
				mNormal[1] = 0; // Clamp to closest side
				if (mVec[1] > 0) {
					mVec[1] = alongY;
				} else {
					mVec[1] = -alongY;
...
gEngine.LayerManager = (function() {
	...
	var addAsShadowCaster = function(obj) {
		...
		for (
			i = 0;
			i < mAllLayers[gEngine.eLayer.eShadowReceiver].size();
			i++
		) {
			mAllLayers[gEngine.eLayer.eShadowReceiver]
				.getObjectAt(i)
				.addShadowCaster(obj);
		}
	};
	...
	var drawAllLayers = function(aCamera) {
		var i;
		for (i = 0; i < kNumLayers; i++) mAllLayers[i].draw(aCamera);
	};
	...
	var updateAllLayers = function() {
		var i;
		for (i = 0; i < kNumLayers; i++) mAllLayers[i].update();
	};
...
gEngine.ResourceMap = (function() {
	...
	var setLoadCompleteCallback = function(func) {
		mLoadCompleteCallback = func; // in case all loading are done
		_checkForAllLoadCompleted();
	};
		...
		if (!gEngine.ResourceMap.isAssetLoaded(fileName)) {
			...
			req.onload = function() {
				...
				if (fileType === eTextFileType.eXMLFile) {
					...
					fileContent = parser.parseFromString(
						req.responseText,
						"text/xml"
					);
				} else {
...
gEngine.DefaultResources = (function() {
	...
	var cleanUp = function() {
		...
		gEngine.TextFileLoader.unloadTextFile(
			kShadowReceiverFS,
			gEngine.TextFileLoader.eTextFileType.eTextFile
		);
		gEngine.TextFileLoader.unloadTextFile(
		...
		if (!gEngine.ResourceMap.isAssetLoaded(clipName)) {
			...
			req.onreadystatechange = function() {
				if (req.readyState === 4 && req.status !== 200) {
					alert(
						clipName +
							": loading failed! Hint: the index.html must beloaded by a webserver"
					);
				}
			};
			/**
...
gEngine.Textures = (function() {
	...
	var activateTexture = function(textureName) {
		...
		// What about the mimap versions of these? Supposed to be "better" // gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST); // gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	};
	...
	var activateNormalMap = function(textureName) {
		...
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE); // Handles how magnification and minimization filters will work.
...
gEngine.Fonts = (function() {
	...
	var getCharInfo = function(fontName, aChar) {
		...
		var commonInfo = fontInfo.evaluate(
			commonPath,
			fontInfo,
			null,
		...
		var charInfo = fontInfo.evaluate(
			charPath,
			fontInfo,
			null,
			XPathResult.ANY_TYPE,
		...
		var bottomPixel =
			topPixel - Number(charInfo.getAttribute("height")) + 1;
		...
		returnInfo.mCharWidthOffset =
			charInfo.getAttribute("xoffset") / charWidth;
		returnInfo.mCharHeightOffset =
			charInfo.getAttribute("yoffset") / charHeight;
...
Light.prototype.setNear = function(n) {
	this.mNear = n;
};
...
Light.prototype.setFar = function(f) {
	this.mFar = f;
};
...
Transform.prototype.getXform = function() {
	...
	var matrix = mat4.create(); // The matrices that WebGL uses are transposed, thus the typical matrix // operations must be in reverse. // Step A: compute translation, for now z is the mHeight

	mat4.translate(matrix, matrix, this.get3DPosition()); // Step B: concatenate with rotation.
	mat4.rotateZ(matrix, matrix, this.getRotationInRad()); // Step C: concatenate with scaling
	mat4.scale(
		matrix,
		matrix,
...
BoundingBox.prototype.boundCollideStatus = function(otherBound) {
	...
	if (this.intersectsBound(otherBound)) {
		...
		if (otherBound.maxY() > this.maxY())
			status |= BoundingBox.eboundCollideStatus.eCollideTop; // if the bounds intersects and yet none of the sides overlaps // otherBound is completely inside thisBound
...
function TextureRenderable(myTexture) {
	...
	this.mTexture = null; // these two instance variables are to cache texture information // for supporting per-pixel accurate collision
	this.mTextureInfo = null;
	this.mColorArray = null; // defined for subclass to override
	this.mTexWidth = 0;
	this.mTexHeight = 0;
...
TextureRenderable.prototype._wcPositionToIndex = function(
	...
	returnIndex[1] = this.mTexHeight * (yDisp / this.mXform.getHeight()); // recall that xForm.getPosition() returns center, yet Texture origin is at lower-left corner!
		...
		while (!pixelTouch && yIndex < this.mTexHeight) {
			...
			if (this._pixelAlphaValue(xIndex, yIndex) > 0) {
				...
				if (
					...
					pixelTouch =
						other._pixelAlphaValue(otherIndex[0], otherIndex[1]) >
						0;
				}
			}
			yIndex++;
 ...
 */
SpriteRenderable.prototype.setElementUVCoordinate = function(
	left,
	right,
	bottom,
	top
 ...
 * @param{number} top - top bound
 */
SpriteRenderable.prototype.setElementPixelPositions = function(
	left,
	right,
	bottom,
	top
...
FontRenderable.prototype.draw = function(vpMatrix) {
	...
	var xPos =
		this.mXform.getXPos() - widthOfOneChar / 2 + widthOfOneChar * 0.5;
...
function LineRenderable(x1, y1, x2, y2) {
	...
	Renderable.prototype._setShader.call(
		this,
		gEngine.DefaultResources.getLineShader()
	);
...
function IllumRenderable(myTexture, myNormalMap) {
	...
	Renderable.prototype._setShader.call(
		this,
		gEngine.DefaultResources.getIllumShader()
	);
...
ShadowCaster.prototype._computeShadowGeometry = function(aLight) {
	...
	if (aLight.getLightType() === Light.eLightType.eDirectionalLight) {
		if (
			Math.abs(aLight.getDirection())[2] < this.kVerySmall ||
		...
		if (
			Math.abs(lgtToReceiverZ) < this.kVerySmall ||
		...
		scale =
			(distToCaster + distToReceiver * this.kReceiverDistanceFudge) /
			distToCaster;
	}
	vec3.scaleAndAdd(
		offset,
...
ShadowReceiver.prototype.draw = function(aCamera) {
	var c; // A: draw receiver as a regular renderable
	...
	this._shadowRecieverStencilOff(); // B3 // C + D: now draw shadow color to the pixels in the stencil that are switched on
	for (c = 0; c < this.mShadowCaster.length; c++)
		this.mShadowCaster[c].draw(aCamera); // switch off stencil checking
...
GameObject.prototype.pixelTouches = function(otherObj, wcTouchPos) {
	...
	if (
		...
		if (
			myRen.getXform().getRotationInRad() === 0 &&
			...
			var myR = Math.sqrt(
				0.5 * mySize[0] * 0.5 * mySize[0] +
					0.5 * mySize[1] * 0.5 * mySize[1]
			);
			var otherR = Math.sqrt(
				0.5 * otherSize[0] * 0.5 * otherSize[0] +
					0.5 * otherSize[1] * 0.5 * otherSize[1]
			);
			var d = [];
			vec2.sub(
				d,
				myRen.getXform().getPosition(),
...
TiledGameObject.prototype._drawTile = function(aCamera) {
	...
	var dx = 0,
		dy = 0; // offset to the lower left corner // left/right boundary?
	if (right < wcLeft) {
		// left of WC left
	...
	var nx = 1,
		ny = 1; // number of times to draw in the x and y directions
...
Particle.prototype.update = function() {
	var dt = gEngine.GameLoop.getUpdateIntervalInSeconds(); //    v += a * dt //    x += v * dt
	// Symplectic Euler
...
Particle.prototype.getColor = function() {
	return this.mPositionMark1.getColor();
};
...
RigidShape.prototype.collidedRectCirc = function(
	rect1Shape,
	circ2Shape,
	collisionInfo
) {
	...
	if (rect1Shape.containsPos(circ2Pos)) {
		isInside = true; // Find closest axis
	...
	if (isInside) {
		//flip normal if inside the rect
 ...
 */
RigidShape.prototype.update = function() {
	var dt = gEngine.GameLoop.getUpdateIntervalInSeconds(); //    v += (1/m * a) * dt //    x += v * dt
...
RigidRectangle.prototype.containsPos = function(pos) {
	...
	return (
		rMinX < pos[0] && rMaxX > pos[0] && (rMinY < pos[1] && rMaxY > pos[1])
	);
};
...
RigidRectangle.prototype.collidedRectRect = function(r1, r2, collisionInfo) {
	...
	if (xDepth > 0) {
		var yDepth =
			r1.getHeight() / 2 + r2.getHeight() / 2 - Math.abs(vFrom1to2[1]);
		if (yDepth > 0) {
...
function SimpleShader(vertexShaderPath, fragmentShaderPath) {
	...
	this.mVertexShader = this._compileShader(
		vertexShaderPath,
		gl.VERTEX_SHADER
	);
	this.mFragmentShader = this._compileShader(
		fragmentShaderPath,
		gl.FRAGMENT_SHADER
	);
	...
	this.mPixelColor = gl.getUniformLocation(
		this.mCompiledShader,
		"uPixelColor"
	);
...
SimpleShader.prototype._compileShader = function(filepath, shaderType) {
	...
	var gl = gEngine.Core.getGL(); // Step A: Get the shader source from the folder
	...
	compiledShader = gl.createShader(shaderType); // Step C: Compile the created shader

	//remember,gGL is the canvas context
	gl.shaderSource(compiledShader, shaderSource);
	gl.compileShader(compiledShader); // Step D: check for errors and return results (null if error) // The log info is how shader compilation errors are typically displayed. // This is useful for debugging the shaders.
...
SimpleShader.prototype.activateShader = function(pixelColor, aCamera) {
	...
	gl.uniform4fv(
		this.mGlobalAmbientColor,
		gEngine.DefaultResources.getGlobalAmbientColor()
	);
	gl.uniform1f(
		this.mGlobalAmbientIntensity,
...
function TextureShader(vertexShaderPath, fragmentShaderPath) {
	...
	var gl = gEngine.Core.getGL();
	this.mShaderTextureCoordAttribute = gl.getAttribLocation(
		this.mCompiledShader,
		"aTextureCoordinate"
	);
...
function LineShader(vertexShaderPath, fragmentShaderPath) {
	...
	// point size uniform
	this.mPointSizeRef = gl.getUniformLocation(
		this.mCompiledShader,
		"uPointSize"
	);
...
ShaderLightAtIndex.prototype._setShaderReferences = function(
	aLightShader,
	index
) {
	var gl = gEngine.Core.getGL();
...
function IllumShader(vertexShaderPath, fragmentShaderPath) {
	...
	this.mMaterial = null;
	this.mMaterialLoader = new ShaderMaterial(this.mCompiledShader); // Reference to the camera position
...
Camera.prototype.setupViewProjection = function() {
	 ...
	 */
	mat4.ortho(
		this.mProjMatrix, //defines the dimesion of the WC
	...
	this.mRenderCache.mCameraPosInPixelSpace[2] = this.fakeZInPixelSpace(
		this.kCameraZ
	);
};
...
Camera.prototype.panWith = function(aXform, zone) {
	...
	if (status !== BoundingBox.eboundCollideStatus.eInside) {
		...
		if ((status & BoundingBox.eboundCollideStatus.eCollideRight) !== 0) {
			newC[0] =
				pos[0] + aXform.getWidth() / 2 - (zone * this.getWCWidth()) / 2;
		}
		if ((status & BoundingBox.eboundCollideStatus.eCollideLeft) !== 0) {
			newC[0] =
				pos[0] - aXform.getWidth() / 2 + (zone * this.getWCWidth()) / 2;
...
Camera.prototype.shake = function(xDelta, yDelta, shakeFrequency, duration) {
	this.mCameraShake = new CameraShake(
		this.mCameraState,
		xDelta,
		yDelta,
		shakeFrequency,
...
Camera.prototype._mouseDCX = function() {
	// Camera.eViewport.eOrgX = 0
	return (
		gEngine.Input.getMousePosX() - this.mViewport[Camera.eViewport.eOrgX]
	);
};

Camera.prototype._mouseDCY = function() {
	// Camera.eViewport.eOrgY = 1
	return (
		gEngine.Input.getMousePosY() - this.mViewport[Camera.eViewport.eOrgY]
	);
};
...
Camera.prototype.isMouseInViewport = function() {
	...
	return (
		dcX >= 0 &&
		dcX < this.mViewport[Camera.eViewport.eWidth] &&
		dcY >= 0 &&
		dcY < this.mViewport[Camera.eViewport.eHeight]
	);
};
...
Camera.prototype.mouseWCX = function() {
	...
	return (
		minWCX +
		this._mouseDCX() *
			(this.getWCWidth() / this.mViewport[Camera.eViewport.eWidth])
	);
};
...
Camera.prototype.mouseWCY = function() {
	...
	return (
		minWCY +
		this._mouseDCY() *
			(this.getWCHeight() / this.mViewport[Camera.eViewport.eHeight])
	);
};
/*
...
Camera.prototype.wcPosToPixel = function(p) {
	var x =
		this.mViewport[Camera.eViewport.eOrgX] +
		(p[0] - this.mRenderCache.mCameraOrgX) *
			this.mRenderCache.mWCToPixelRatio +
		0.5;
	var y =
		this.mViewport[Camera.eViewport.eOrgY] +
